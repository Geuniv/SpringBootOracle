...
public class WeatherService implements IWeatherService {
    ...
    public int toDayWeather() throws Exception {
        ...
        String url = "https://weather.naver.com/today/09500103";
        ...
        // 네이버 날씨 웹페이지의 전체 소스 중 일부태그를 선택하기 위해 사용
        // <div class="is_today.korean"> 이 태그 내에서 있는 HTML 소스만 element에 저장됨
        Elements elements = doc.select("div.is_today.korean");
        
        // Iterator 을 사용하여 영화 순위 정보를 가져오기
        // 영화순위는 기본적으로 1개 이상의 영화가 존재하기 때문에 태그의 반복이 존재할 수 밖에 없음
        Iterator<Element> region = elements.select("strong.location_name").iterator(); // 지역
        Iterator<Element> temp = elements.select("strong.current").iterator(); // 온도
        Iterator<Element> weather = elements.select("span.weather").iterator(); // 지역
        ...
        while (region.hasNext()) {
            ...
            // 날씨 정보 ( trim 함수 추가 이유 : trim 함수는 글자의 앞 뒤 공백 삭제역할을 수행하여, 데이터 수집시,
            // 홈페이지 개발자들을 암ㅍ뒤 공백 집어넣을 수 있어서 추가 )
            String reg = CmmUtil.nvl(region.next().text()).trim(); // No.1 들어옴

            // 지역 정보
            pDTO.setRegion(CmmUtil.nvl(region.next().text()).trim());
